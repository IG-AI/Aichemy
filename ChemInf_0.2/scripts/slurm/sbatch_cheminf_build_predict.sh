...
#SBATCH --job-name=real_case_cheminf
...
python cheminf/ preproc balancing -i $input -n $name -cf $config_override -ch 100000 -nc 4

srun -N1 --ntasks=1 --cpus-per-task=4 --mem=300G --time=24:00:00 --exclusive \
singularity exec /proj/carlssonlab/singularity/frontline.simg \
python cheminf/ preproc sample -i $input -n $name -cf $config_override -pc $percentage -ch 100000

srun -N1 --ntasks=1 --cpus-per-task=4 --mem=300G --time=24:00:00 --exclusive \
singularity exec /proj/carlssonlab/singularity/frontline.simg \
python cheminf/ build -cl nn -i "cheminf/data/"$name"/"$filename"_balanced."$extension -n $name -cf $config_override
...
# sbatch --export=ALL,SETUP=scripts/slurm/projects/complement.csv scripts/slurm/sbatch_cheminf_build_predict.sh